...
use crate::discord::{new_discord_protocol, MessageType, Msg};
...
impl P2P {
    pub fn new(token: String, channel_id: String) -> P2P {
        ...
        tokio::spawn(async move {
            new_discord_protocol(token, channel_id, ch.clone().one.0, ch.clone().two.1)
                .await
                .unwrap()
                .start()
                .await
        ...
        let data = {
            if let Some(timeout) = timeout {
                self.channel
                    .receive_with_timeout(
                        timeout,
                        self.other_end.clone().unwrap().as_str().trim(),
                        None,
                    )
                    .await?
            } else {
                self.channel
                    .receive(self.other_end.clone().unwrap().as_str().trim(), None)
                    .await?
            }
        };
